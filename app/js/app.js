...
//	when('/details/eventlog/:id', 'details.eventlog').
	...
	when('/gps/dw_2', 'experiments_dw2').
	when('/gps/dw_3', 'experiments_dw3').
	when('/gps/dw_4', 'experiments_dw4').
...
//	when('/public/alarm/:id', 'report.alarm').
	...
	within(). // account
		...
		segment('settings', {templateUrl: 'partials/account/settings.html',
			resolve : {
				parseUser : function() {
					return Parse.User.current().fetch();
				}
...
//	    within(). // crud
    ...
    within()
		...
		.segment('history', {templateUrl: 'partials/crud/data/client/client.history.html', controller: 'ParseSearchWithQueryCtrl',
			resolve : {
		...
		.segment('statistics', {templateUrl: 'partials/crud/data/client/client.statistics.html', controller: 'ParseSearchWithQueryCtrl',
			resolve : {
				ParseObject : ['ParseEventLog', function(ParseEventLog) {return ParseEventLog}],
				parseQuery : ['ParseEventLog', 'ParseClient', function(ParseEventLog, ParseClient) {
					var query = ParseEventLog.fetchAllQuery();
...
//	within(). // log
	 ...
	 */
		...
		segment('alarmeventlog', {templateUrl: 'partials/logs/eventlog.html', dependencies: ['id'], controller: 'ParseLogResultCtrl',
			resolve : {
				ParseObject : ['ParseEventLog', function(ParseEventLog) {return ParseEventLog}],
		...
		// regular tasks
		segment('circuitlog', {templateUrl: 'partials/logs/circuitlog.html', controller: 'StandardParseSearchCtrl',
			resolve : {
				ParseObject : ['ParseCircuitStarted', function(ParseCircuitStarted) {return ParseCircuitStarted}]
			}
		}).
		// regular tasks list events of selected circuit
		segment('circuiteventlog', {templateUrl: 'partials/logs/eventlog.html', dependencies: ['id'], controller: 'ParseLogResultCtrl',
			resolve : {
				ParseObject : ['ParseEventLog', function(ParseEventLog) {return ParseEventLog}],
		...
		// districtwatches
		segment('districtwatchlog', {templateUrl: 'partials/logs/districtwatchlog.html', controller: 'StandardParseSearchCtrl',
			resolve : {
				ParseObject : ['ParseDistrictWatchStarted', function(ParseDistrictWatchStarted) {return ParseDistrictWatchStarted}],
		...
		segment('districtwatcheventlog', {templateUrl: 'partials/logs/eventlog.html', dependencies: ['id'], controller: 'ParseLogResultCtrl',
			resolve : {
				ParseObject : ['ParseEventLog', function(ParseEventLog) {return ParseEventLog}],
...
//		segment('listreports', {templateUrl: 'partials/logs/reportslog.html', controller: 'ReportListCtrl'}).
		...
		segment('alarmreports', {templateUrl: 'partials/report/alarm_list_reports.html', controller: 'StandardParseSearchCtrl',
			resolve : {
				ParseObject : ['ParseAlarm', function(ParseAlarm) {return ParseAlarm}]
			}
		}).
		segment('alarmreport', {templateUrl: 'partials/report/alarm_report.html', dependencies: ['id'], controller: 'AlarmReportCtrl',
			resolve : {
	  	...
	  	segment('circuitunitreports', {templateUrl: 'partials/report/circuitunit_list_reports.html', controller: 'ParseSearchWithQueryCtrl',
				resolve : {
		 ...
		 * --
		 */
		segment('gps', {templateUrl: 'partials/gps/gps.html', controller: 'GPSLogCtrl',
			resolve : {
				ParseObject : ['ParseGPSTracker', function(ParseGPSTracker) {return ParseGPSTracker}],
				parseQuery : ['ParseGPSTracker', function(ParseGPSTracker) {return ParseGPSTracker.fetchAllQuery().include('guard')}]
			}
		}).
		segment('experiments_regular_panel', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
			resolve : {
				gpsData : ['$http', function($http) {return $http.get('experiments/regular/instrument_panel/gps.json')}],
				eventData : ['$http', function($http) {return $http.get('experiments/regular/instrument_panel/events.json')}],
				groundtruthData : ['$http', function($http) {return $http.get('experiments/regular/instrument_panel/groundtruth.json')}],
				clientData : ['$http', function($http) {return $http.get('experiments/regular/clients.json')}]
			}
		}).
		segment('experiments_regular_pocket', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
			resolve : {
				gpsData : ['$http', function($http) {return $http.get('experiments/regular/left_pant_pocket/gps.json')}],
				eventData : ['$http', function($http) {return $http.get('experiments/regular/left_pant_pocket/events.json')}],
				groundtruthData : ['$http', function($http) {return $http.get('experiments/regular/left_pant_pocket/groundtruth.json')}],
				clientData : ['$http', function($http) {return $http.get('experiments/regular/clients.json')}]
			}
		}).
		segment('experiments_regular_upper', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
			resolve : {
				gpsData : ['$http', function($http) {return $http.get('experiments/regular/upper_jacket_pocket/gps.json')}],
				eventData : ['$http', function($http) {return $http.get('experiments/regular/upper_jacket_pocket/events.json')}],
				groundtruthData : ['$http', function($http) {return $http.get('experiments/regular/upper_jacket_pocket/groundtruth.json')}],
				clientData : ['$http', function($http) {return $http.get('experiments/regular/clients.json')}]
			}
		}).
	segment('experiments_dw1', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
		resolve : {
			gpsData : ['$http', function($http) {return $http.get('experiments/districtwatch/1/gps.json')}],
			eventData : ['$http', function($http) {return $http.get('experiments/districtwatch/1/events.json')}],
			groundtruthData : ['$http', function($http) {return $http.get('experiments/districtwatch/1/groundtruth.json')}],
			clientData : ['$http', function($http) {return $http.get('experiments/districtwatch/1/clients.json')}]
		}
	}).
	segment('experiments_dw2', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
		resolve : {
			gpsData : ['$http', function($http) {return $http.get('experiments/districtwatch/2/gps.json')}],
			eventData : ['$http', function($http) {return $http.get('experiments/districtwatch/2/events.json')}],
			groundtruthData : ['$http', function($http) {return $http.get('experiments/districtwatch/2/groundtruth.json')}],
			clientData : ['$http', function($http) {return $http.get('experiments/districtwatch/2/clients.json')}]
		}
	}).
	segment('experiments_dw3', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
		resolve : {
			gpsData : ['$http', function($http) {return $http.get('experiments/districtwatch/3/gps.json')}],
			eventData : ['$http', function($http) {return $http.get('experiments/districtwatch/3/events.json')}],
			groundtruthData : ['$http', function($http) {return $http.get('experiments/districtwatch/3/groundtruth.json')}],
			clientData : ['$http', function($http) {return $http.get('experiments/districtwatch/3/clients.json')}]
		}
	}).
	segment('experiments_dw4', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
		resolve : {
			gpsData : ['$http', function($http) {return $http.get('experiments/districtwatch/4/gps.json')}],
			eventData : ['$http', function($http) {return $http.get('experiments/districtwatch/4/events.json')}],
			groundtruthData : ['$http', function($http) {return $http.get('experiments/districtwatch/4/groundtruth.json')}],
			clientData : ['$http', function($http) {return $http.get('experiments/districtwatch/4/clients.json')}]
		}
	}).
	segment('experiments_dw5', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
		resolve : {
			gpsData : ['$http', function($http) {return $http.get('experiments/districtwatch/5/gps.json')}],
			eventData : ['$http', function($http) {return $http.get('experiments/districtwatch/5/events.json')}],
			groundtruthData : ['$http', function($http) {return $http.get('experiments/districtwatch/5/groundtruth.json')}],
			clientData : ['$http', function($http) {return $http.get('experiments/districtwatch/5/clients.json')}]
		}
	}).
	segment('experiments_dw6', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
		resolve : {
			gpsData : ['$http', function($http) {return $http.get('experiments/districtwatch/6/gps.json')}],
			eventData : ['$http', function($http) {return $http.get('experiments/districtwatch/6/events.json')}],
			groundtruthData : ['$http', function($http) {return $http.get('experiments/districtwatch/6/groundtruth.json')}],
			clientData : ['$http', function($http) {return $http.get('experiments/districtwatch/6/clients.json')}]
		}
	}).
	segment('experiments_dw7', {templateUrl: 'partials/gps/experiments.html', controller: 'ExperimentsCtrl',
		resolve : {
			gpsData : ['$http', function($http) {return $http.get('experiments/districtwatch/7/gps.json')}],
			eventData : ['$http', function($http) {return $http.get('experiments/districtwatch/7/events.json')}],
			groundtruthData : ['$http', function($http) {return $http.get('experiments/districtwatch/7/groundtruth.json')}],
